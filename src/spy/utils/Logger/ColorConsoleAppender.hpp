...
#pragma warning(suppress : 26812) //  Prefer 'enum class' over 'enum'
        ...
        virtual void write(const Record &record) {
            ...
            switch (record.getSeverity()) {
                case fatal:
                    this->m_outputStream << "F";
                    break;

                case error:
                    this->m_outputStream << "E";
                    break;

                case warning:
                    this->m_outputStream << "W";
                    break;

                case info:
                    this->m_outputStream << "I";
                    break;

                case debug:
                    this->m_outputStream << "D";
                    break;

                case verbose:
                    this->m_outputStream << "V";
                    break;
...
#else
                case fatal:
                    this->m_outputStream << "\x1B[;37m\x1B[;41m"; // white on red background
                    break;

                case error:
                    this->m_outputStream << "\x1B[1;31m"; // red
                    break;

                case warning:
                    this->m_outputStream << "\x1B[;33m"; // yellow
                    break;
